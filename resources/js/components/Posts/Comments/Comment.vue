			...
			<div class="media pb-3" :class="{'mb-3 bb1px':(index!=comments.length-1)}">
				...
				<div class="media-body">
					...
					<div class="d-flex mt-10">
						...
						<span v-if="authUser==comment.user.id" class="ml-2 mr-2">·</span>
						<a v-if="authUser==comment.user.id" href="#" class="font-14 text-light text-capitalize font-weight-500" @click.prevent="editComment(index,comment)">edit</a>
						<span v-if="authUser==comment.user.id" class="ml-2 mr-2">·</span>

						<a v-if="authUser==comment.user.id"  href="#" class="font-14 text-light text-capitalize font-weight-500" @click.prevent="deleteComment(index,comment.id)">Delete</a>
					</div>
				...
				.then((willDelete) => {
				  	if (willDelete) {
						...
						.then(res =>{
							...
							this.$store.commit('deleteComment',this.postIndex);
		...
		created(){
			...
			eventBus.$on('commentCreated',(data) => {
				if (this.type!='profile') {
					...
					this.$store.commit('addComment',data.index);
				}
			})
